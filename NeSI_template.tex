...
\author{Center for e-Research \\(eresearch@nesi.org.nz)}
\date{}
...
{
  \frametitle{About Us}
     \begin{block}{CER: Center for e-Research}
   \begin{itemize}%[<+-| alert@+>]
...
{
  \frametitle{Submitting a Job}
   \begin{block}{Two (and a half) main tools for submitting a job}
   \begin{itemize}%[<+-| alert@+>]
	\item LoadLeveler (LL) – for people comfortable with the Linux command line
	\item SLURM: will replace LL
	\item Grisu Template Client – for a graphical interface
   \end{itemize}
  \end{block}
   \begin{block}{Which one to use? In general}
   \begin{itemize}%[<+-| alert@+>]
	\item LL/SLURM for complex workflows or large numbers of jobs
	\item Grisu for very simple workflows or few jobs 
   \end{itemize}
  \end{block}
}
...
\subsection{LoadLeveler and SLURM}
\frame[t]
{
  \frametitle{Submitting a Job}
      \begin{block}{Quick Intro to LoadLeveler and Slurm}
   \begin{itemize}%[<+-| alert@+>]
...
\frame[t]
{
  \frametitle{Submitting a Job with LoadLeveler: Send the job to the queue}
...
\frame[t]
{
  \frametitle{Submitting a Job}
   ...
   \begin{itemize}%[<+-| alert@+>]
	...
	\item A wrong memory request or a wrong job description setup can potentially affect others. (Should be less a problem with SLURM.)
